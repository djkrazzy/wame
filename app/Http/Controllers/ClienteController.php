...
class ClienteController extends VoyagerBaseController
{
    ...
    public function index(Request $request)
    {
        ...
        if (view()->exists("voyager::$slug.browse")) {
         ...
         if ($request->ajax()) {
           ...
           if( $request->has('jornada') && !empty( $request->get('jornada'))) {
             //$consulta= $data->where('estado', $request->get('estado')); 
             $data = Cliente::where('jornada', $request->get('jornada'))->orderBy('created_at','DESC')->get();  
            } else{
                $data = Cliente::get()->where('jornada', NULL);
            }
             ...
             return DataTables::of($data)
                 ...
                 ->addColumn('action', function($user){
                    ...
                    <a href="javascript:;" title="Borrar" class="btn btn-sm btn-danger pull-right delete" data-id="'.$user->id.'" id="delete-'.$user->id.'">
                <i class="voyager-trash"></i> <span class="hidden-xs hidden-sm">Borrar</span>
            </a>
                    <a href="'.route("voyager.clientes.show", $user).'" title="Ver" class="btn btn-sm btn-warning pull-right view">
                ...
                ->editColumn('jornada', function ($data) {  
                    return  ($data->jornada =='1') ? '<span class="label label-info">Jornada</span>':  '<span class="label label-primary">Clinica</span>' ;  
                })
                ->editColumn('telefono', function ($data) {  
                    return  '<a href="https://api.whatsapp.com/send?phone=502'.$data->telefono.'&text=Buen%20dia'.$data->nombres.'" target="_blank" >'.$data->telefono.' </a> ' ;  
                })
              
              
              //  ->filterColumn('marca', function ($query, $keyword) {
                   
                  //  $query->whereRaw("marca_id like ?", ["%$keyword%"]);
               // })
              //  ->filter( function ($query) use ($request) {
                 ...
                 //   }       
              //  })
                 ->rawColumns(['action','jornada','telefono'])
    ...
    public function show(Request $request, $id)
    {
        ...
        $nacimiento = $dataTypeContent->nacimiento;
        $edad = Carbon::parse($nacimiento )->age; 
       
       
        return Voyager::view($view, compact('dataType', 'dataTypeContent', 'isModelTranslatable', 'isSoftDeleted','edad'));
    }
    ...
    public function store(Request $request)
    {
       ...
       $ardor=isset($request->ardor)? 1:0;
       $dolor_cabeza=isset($request->dolor_cabeza)? 1:0;
       $irrtitacion=isset($request->irrtitacion)? 1:0;
       $prurito=isset($request->prurito)? 1:0;
       $sensibilidad=isset($request->sensibilidad)? 1:0;
       $dolor_ocular=isset($request->dolor_ocular)? 1:0;
       $sombras=isset($request->sombras)? 1:0;
       $cataratas=isset($request->cataratas)? 1:0;
       $glaucoma=isset($request->glaucoma)? 1:0;
       $pterigion=isset($request->pterigion)? 1:0;
       $vision_borrosa=isset($request->vision_borrosa)? 1:0;
       $halos=isset($request->halos)? 1:0;
       $deslumbra=isset($request->deslumbra)? 1:0;
       $mareos=isset($request->mareos)? 1:0;
       $inflamacion=isset($request->inflamacion)? 1:0;
       $lagrimeo=isset($request->lagrimeo)? 1:0;
       $otros=isset($request->otros)? 1:0;
      ...
      $consulta= Consulta::create([
        ...
        'irrtitacion' => $irrtitacion,
        'prurito' => $prurito,
        'sensibilidad' => $sensibilidad,
        'dolor_ocular' => $dolor_ocular,
        'sombras' => $sombras,
        'cataratas' => $cataratas,
        'glaucoma' => $glaucoma,
        'pterigion' => $pterigion,
