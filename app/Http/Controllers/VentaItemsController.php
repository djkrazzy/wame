...
class VentaItemsController extends VoyagerBaseController
{
    ...
    public function __construct()
{
                ...
                ->filter(function ($query) use ($request) {
                    ...
                    if (!empty($request->get('start_date')) && !empty($request->get('end_date'))) {
                       ...
                       $query->whereBetween('created_at', [$request->get('start_date') ,$request->get('end_date')]);
                        //$query->whereRaw('date(created_at) between '.$request->get("start_date").' and '.$request->get("end_date"));
                    }
                } )
    ...
    public function create(Request $request)
    {
        ...
        return view('vendor.voyager.venta-items.create',compact('articulos','cliente','orden'));
    }
    ...
    public function store(Request $request)
    {
       //return $request->all();
        ...
        $trabajo =  Trabajo::findOrFail($request->orden_id);
        $data = VentaItems::where('trabajo_id', $request->orden_id)->get();  
        ...
        $i=0;
            ...
            VentaItems::create([
                ...
                'descuento' => $request->descuento_venta,
                'total' => $request->precio -$request->descuento_venta,
                'trabajo_id' => $request->orden_id,
                'cliente_id' => $request->cliente_id,
                'articulo_id' => null,
             ]);

         
               // $articulo_edit = Articulo::findOrFail($request->orden_id);

                //if($articulo_edit->stok){
               // $articulo_edit->cantidad =  $articulo_edit->cantidad - $request->cantidad ; 
                //guardamos todo en la cuenta
            //  }
              
              if($request->precio >= $trabajo->deuda_cliente){
               // $trabajo->estado=3;//entregar
                $trabajo->deuda_cliente=$trabajo->deuda_cliente-$request->precio;
               // Articulo::where('user_id',1)->where('is_read',0)->update(['is_read'=>1]);
               foreach ($data as $row) {
                Articulo::where('id', $row->articulo_id)->update(['estado'=>3]);
        ...
        return redirect('admin/clientes/'.$request->cliente_id)->with('info', 'Pago Agregado');
    }
