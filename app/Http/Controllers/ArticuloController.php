...
class ArticuloController extends VoyagerBaseController
{
        ...
        if (strlen($dataType->model_name) != 0) {
            ...
            if ($orderBy && (in_array($orderBy, $dataType->fields()) || !empty($row))) {
                ...
                $dataTypeContent = call_user_func([
                    $query->orderBy($orderBy, $querySortOrder)->with( 'categoria'  )->select('*'),
                    $getter,
                ]);
         ...
         if ($request->ajax()) {
           ...
           if( $request->has('estado') && !empty( $request->get('estado'))) {
             ...
             if ($request->get('estado')==null) {
                $data = Articulo::with( 'categoria','marca','material')->select('*')->where('estado', null)->orderBy('created_at','DESC');  

             }else{
             $data = Articulo::with( 'categoria','marca','material')->select('*')->where('estado', $request->get('estado'))->orderBy('created_at','DESC');  
                }
            } else{
                $data = Articulo::with( 'categoria','marca','material')->select('*')->orderBy('created_at','DESC');
            }
             return Datatables::of($data)
                 ...
                 ->addColumn('action', function($user){
                    $actionBtn = '
                    <a href="'.route("voyager.articulos.edit", $user).'" title="Editar" class="btn btn-sm btn-primary pull-right edit">
                ...
                <i class="voyager-trash"></i> <span class="hidden-xs hidden-sm">Borrar</span>
                 ...
                 ->editColumn('estado', function ($data) {
                    ...
                    switch ($data->estado ) {
                        case '1':
                            return '<span class="badge badge-succes">Stock</span>';
                            break;
                        
                            case '2':
                            return '<span class="badge badge-warning">Apartado</span>';
                            break;

                            case '3':
                           return '<span class="badge badge-danger">Entregado</span>';
                                break;
                ...
                ->editColumn('marca', function ($data) {
                    return  $data->marca ? $data->marca->nombre:  'sin datos' ;
                })
                ->editColumn('material', function ($data) {  
                    return  $data->material ? $data->material->nombre:  'sin datos' ;  
                })
                ->editColumn('stock', function ($data) {  
                    return  ($data->stok =='1') ? '<span class="label label-info">Stok</span>':  '<span class="label label-primary">Extra</span>' ;  
                })
                ->editColumn('unico', function ($data) {  
                    return  ($data->unico =='1') ? '<span class="label label-info">Unico</span>':  '<span class="label label-primary">No</span>' ;  
                })
                ...
                ->editColumn('imagen', function ($data) {  
                    return '<img src="'.Storage::url($data->imagen).'" style="width:25px">' ;  
                })
                ->editColumn('nombre', function ($data) {  
                    return '<a href="'.route("voyager.articulos.show", $data).'" >'.$data->nombre.'</a>' ;  
                })
              ...
              //  ->filter( function ($query) use ($request) {
                 ...
                 //   }       
              //  })
                 ->rawColumns(['action','estado','marca','material','stock','unico','imagen','nombre','jornada'])
                 ->make(true);
